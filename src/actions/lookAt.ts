...
export class LookAt implements Action {
  ...
  private _shortestPathIsPositive: boolean = false;
  ...
  public update(_delta: number): void {
    if (!this._started) {
      ...
      this._shortestPathIsPositive =
        (this._start - this._end + TwoPI) % TwoPI >= Math.PI;
    ...
    this._currentNonCannonAngle +=
      this._direction * this._speed * (_delta / 1000);
  ...
  public isComplete(): boolean {
    const distanceTraveled = Math.abs(
      this._currentNonCannonAngle - this._start
    );
